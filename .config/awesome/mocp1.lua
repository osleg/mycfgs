...
function init(args)
    ...
    moctimer:connect_signal("timeout", scroller)
    moctimer:start()

    -- on mouseenter, we delay before showing the popup info
    mocpoptimer = timer { timeout = 2 }
    mocpoptimer:connect_signal("timeout", popup)

    -- what to do when mouse over
    config.widget:connect_signal("mouse::enter", function() mocpoptimer:start() end)
    config.widget:connect_signal("mouse::leave", function() mocpoptimer:stop() end)
